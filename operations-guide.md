...
| `Чернетки викладачів` | `/teacher-drafts` | Попереднє планування занять силами викладачів; інтеграція з `TeacherDraftsController`. |
  - `GET /api/teacher-drafts/export` використовує ті ж `weekStart`/`teacherId`/`groupId`/`roomId`, формує Excel із днями/слотами/групами, додає заголовки і фільтри з назвою тижня, зберігає в `MemoryStream` і повертає `Rozklad-yyyyMMdd.xlsx` з `application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`.  
...
- Ви можете створювати модуль, вказуючи код, назву, курс та характеристику (кредити).  
...
- Видалення модуля перевіряє використання в розкладі. Якщо таблиця `ScheduleItems` містить записи, система вимагатиме підтвердити `force=true`.
...
- `TeacherDrafts.razor` забезпечує drag-and-drop інтерфейс. Ви можете:
  ...
  - Кнопка "Завантажити в Excel" у `toolbar-actions` викликає `ExportScheduleAsync`, блокує повторні натискання (`_isExporting`), передає `weekStart/teacherId/groupId/roomId`, викликає `DraftsApi.ExportWeek`, а байти передає до `downloadFromBytes` (`download.js`), щоб зберегти `Rozklad-yyyyMMdd.xlsx`.  
- API:  
...
4. **Запросіть викладачів внести чернетки** на сторінці `/teacher-drafts`. Вони підбирають теми, узгоджують тайм-слоти, перевіряють конфлікти, а кнопка "Завантажити в Excel" формує актуальний `.xlsx` для передачі викладачам або адміністрації.  
...
- **EF Core**: форма міграцій описана в `README.md`. Базова міграція — `20251106110134_InitialCreate`.  
...
- **DTO та мапінг**: усі типи для обміну даними між клієнтом і сервером зібрані в `Shared/DTOs`.  
